...
add_action( 'sbf_init', 'sbf_add_form_tag_checkbox', 10, 0 );

function sbf_add_form_tag_checkbox() {
	sbf_add_form_tag( array( 'checkbox', 'checkbox*', 'radio' ),
		'sbf_checkbox_form_tag_handler',
		array(
			'name-attr' => true,
...
function sbf_checkbox_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
	...
	$hangover = sbf_get_hangover( $tag->name, $multiple ? array() : '' );
	...
	foreach ( $values as $key => $value ) {
		...
		if ( $label_first ) { // put label first, input last
			$item = sprintf(
				'<span class="sbf-list-item-label">%1$s</span><input %2$s />',
				esc_html( $label ), $item_atts
			);
		} else {
			$item = sprintf(
				'<input %2$s /><span class="sbf-list-item-label">%1$s</span>',
				esc_html( $label ), $item_atts
			);
		}
		...
		if ( count( $values ) == $count ) { // last round
			...
			if ( $free_text ) {
				...
				and isset( $_POST[$free_text_name] ) ) {
					$free_text_post_data = wp_unslash($_POST[$free_text_name]);
					if (is_array($free_text_post_data)) {
						$free_text_post_data = array_map('sanitize_text_field', $free_text_post_data);
					} else {
						$free_text_post_data = sanitize_text_field($free_text_post_data);
					}
					$free_text_atts['value'] = $free_text_post_data;
				}

				$free_text_atts = sbf_format_atts( $free_text_atts );
...
add_filter( 'sbf_validate_checkbox',
	'sbf_checkbox_validation_filter', 10, 2 );
add_filter( 'sbf_validate_checkbox*',
	'sbf_checkbox_validation_filter', 10, 2 );
add_filter( 'sbf_validate_radio',
	'sbf_checkbox_validation_filter', 10, 2 );

function sbf_checkbox_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
add_action( 'sbf_admin_init',
	'sbf_add_tag_generator_checkbox_and_radio', 30, 0 );

function sbf_add_tag_generator_checkbox_and_radio() {
	$tag_generator = SBF_TagGenerator::get_instance();
	$tag_generator->add( 'checkbox', __( 'checkboxes', 'salesbox-crm-form' ),
		'sbf_tag_generator_checkbox' );
	$tag_generator->add( 'radio', __( 'radio buttons', 'salesbox-crm-form' ),
		'sbf_tag_generator_checkbox' );
}

function sbf_tag_generator_checkbox( $contact_form, $args = '' ) {
	...
	if ( 'checkbox' == $type ) {
		$description = __( "Generate a form-tag for a group of checkboxes. For more details, see %s.", 'salesbox-crm-form' );
	} elseif ( 'radio' == $type ) {
		$description = __( "Generate a form-tag for a group of radio buttons. For more details, see %s.", 'salesbox-crm-form' );
	}
...
<?php endif; ?>
	...
	<td>
		...
		<label><input type="checkbox" name="label_first" class="option" /> <?php echo esc_html( __( 'Put a label first, a checkbox last', 'salesbox-crm-form' ) ); ?></label><br />
		<label><input type="checkbox" name="use_label_element" class="option" checked="checked" /> <?php echo esc_html( __( 'Wrap each item with label element', 'salesbox-crm-form' ) ); ?></label>
<?php if ( 'checkbox' == $type ) : ?>
		<br /><label><input type="checkbox" name="exclusive" class="option" /> <?php echo esc_html( __( 'Make checkboxes exclusive', 'salesbox-crm-form' ) ); ?></label>
<?php endif; ?>
		</fieldset>
	</td>
	</tr>
