...
class SBF_ConfigValidator {
	...
	public static function get_doc_link( $error_code = '' ) {
		$url = __( 'https://salesbox.com/configuration-errors/',
			'salesbox-crm-form' );
	...
	public function get_default_message( $code ) {
		switch ( $code ) {
			case self::error_maybe_empty:
				return __( "There is a possible empty field.", 'salesbox-crm-form' );
			case self::error_invalid_mailbox_syntax:
				return __( "Invalid mailbox syntax is used.", 'salesbox-crm-form' );
			case self::error_email_not_in_site_domain:
				return __( "Sender email address does not belong to the site domain.", 'salesbox-crm-form' );
			case self::error_html_in_message:
				return __( "HTML tags are used in a message.", 'salesbox-crm-form' );
			case self::error_multiple_controls_in_label:
				return __( "Multiple form controls are in a single label element.", 'salesbox-crm-form' );
			case self::error_invalid_mail_header:
				return __( "There are invalid mail header fields.", 'salesbox-crm-form' );
			case self::error_deprecated_settings:
				return __( "Deprecated settings are used.", 'salesbox-crm-form' );
			default:
				return '';
		}
	}
	...
	public function validate() {
		...
		do_action( 'sbf_config_validator_validate', $this );
	...
	public function replace_mail_tags_with_minimum_input( $matches ) {
		...
		$mail_tag = new SBF_MailTag( $tag, $tagname, $values );
		...
		if ( $form_tags ) {
			$form_tag = new SBF_FormTag( $form_tags[0] );
			...
			if ( sbf_form_tag_supports( $form_tag->type, 'selectable-values' ) ) {
				if ( $form_tag->pipes instanceof SBF_Pipes ) {
				...
				if ( $last_item and sbf_is_mailbox_list( $last_item ) ) {
			...
			// for back-compat
			$field_name = preg_replace( '/^sbf\./', '_', $field_name );
	...
	public function detect_multiple_controls_in_label( $section, $content ) {
		...
		if ( preg_match_all( $pattern, $content, $matches ) ) {
			$form_tags_manager = SBF_FormTagsManager::get_instance();
			...
			foreach ( $matches[1] as $insidelabel ) {
				...
				foreach ( $tags as $tag ) {
					$is_multiple_controls_container = sbf_form_tag_supports(
						$tag->type, 'multiple-controls-container' );
					$is_zero_controls_container = sbf_form_tag_supports(
			...
			return $this->add_error( $section,
				...
				array(
					'message' =>
						...
						__( "Unavailable names (%names%) are used for form controls.", 'salesbox-crm-form' ),
	...
	public function detect_unavailable_html_elements( $section, $content ) {
		...
		if ( preg_match( $pattern, $content ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' => __( "Unavailable HTML elements are used in the form template.", 'salesbox-crm-form' ),
	...
	public function validate_mail( $template = 'mail' ) {
			...
			if ( ! preg_match( '/^([0-9A-Za-z-]+):(.*)$/', $header, $matches ) ) {
				...
				if ( in_array( strtolower( $header_name ), $mailbox_header_types ) ) {
					$this->detect_invalid_mailbox_syntax(
						...
						$header_value, array(
							'message' =>
								__( "Invalid mailbox syntax is used in the %name% field.", 'salesbox-crm-form' ),
		...
		$body = new SBF_MailTaggedText( $body,
			array( 'callback' => $callback ) );
		...
		if ( '' !== $components['attachments'] ) {
			...
			if ( $max < $total_size ) {
				$this->add_error( sprintf( '%s.attachments', $template ),
					...
					array(
						'message' => __( "The total size of attachment files is too large.", 'salesbox-crm-form' ),
	...
	public function detect_invalid_mailbox_syntax( $section, $content, $args = '' ) {
		...
		if ( ! sbf_is_mailbox_list( $content ) ) {
			return $this->add_error( $section,
	...
	public function detect_file_not_found( $section, $content ) {
		...
		or ! is_file( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "Attachment file does not exist at %path%.", 'salesbox-crm-form' ),
	...
	public function detect_file_not_in_content_dir( $section, $content ) {
		...
		if ( ! sbf_is_file_path_in_content_dir( $path ) ) {
			return $this->add_error( $section,
				self::error_file_not_in_content_dir,
				array(
					'message' =>
						__( "It is not allowed to use files outside the wp-content directory.", 'salesbox-crm-form' ),
