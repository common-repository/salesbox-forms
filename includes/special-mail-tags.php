...
add_filter( 'sbf_special_mail_tags', 'sbf_special_mail_tag', 10, 4 );
 ...
 * @param SBF_MailTag $mail_tag An object representation of the mail-tag.
 * @return string Output of the given special mail-tag.
 */
function sbf_special_mail_tag( $output, $name, $html, $mail_tag = null ) {
	if ( ! $mail_tag instanceof SBF_MailTag ) {
		sbf_doing_it_wrong(
			sprintf( '%s()', __FUNCTION__ ),
			__( 'The fourth parameter ($mail_tag) must be an instance of the SBF_MailTag class.', 'salesbox-crm-form' ),
			'5.2.2'
		);
	}

	$name = preg_replace( '/^sbf\./', '_', $name ); // for back-compat
...
add_filter( 'sbf_special_mail_tags', 'sbf_post_related_smt', 10, 4 );
 ...
 * @return string Output of the given special mail-tag.
 */
function sbf_post_related_smt( $output, $name, $html, $mail_tag = null ) {
	if ( ! $mail_tag instanceof SBF_MailTag ) {
		sbf_doing_it_wrong(
			sprintf( '%s()', __FUNCTION__ ),
			__( 'The fourth parameter ($mail_tag) must be an instance of the SBF_MailTag class.', 'salesbox-crm-form' ),
...
add_filter( 'sbf_special_mail_tags', 'sbf_site_related_smt', 10, 4 );
 ...
 * @return string Output of the given special mail-tag.
 */
function sbf_site_related_smt( $output, $name, $html, $mail_tag = null ) {
	if ( ! $mail_tag instanceof SBF_MailTag ) {
		sbf_doing_it_wrong(
			sprintf( '%s()', __FUNCTION__ ),
			__( 'The fourth parameter ($mail_tag) must be an instance of the SBF_MailTag class.', 'salesbox-crm-form' ),
...
add_filter( 'sbf_special_mail_tags', 'sbf_user_related_smt', 10, 4 );
 ...
 * @return string Output of the given special mail-tag.
 */
function sbf_user_related_smt( $output, $name, $html, $mail_tag = null ) {
	if ( ! $mail_tag instanceof SBF_MailTag ) {
		sbf_doing_it_wrong(
			sprintf( '%s()', __FUNCTION__ ),
			__( 'The fourth parameter ($mail_tag) must be an instance of the SBF_MailTag class.', 'salesbox-crm-form' ),
