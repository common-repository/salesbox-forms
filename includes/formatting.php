...
function sbf_autop( $pee, $br = 1 ) {
	...
	/* sbf: remove select and input */
	$allblocks = '(?:table|thead|tfoot|caption|col|colgroup|tbody|tr|td|th|div|dl|dd|dt|ul|ol|li|pre|form|map|area|blockquote|address|math|style|p|h[1-6]|hr|fieldset|legend|section|article|aside|hgroup|header|footer|nav|figure|figcaption|details|menu|summary)';
	$pee = preg_replace( '!(<' . $allblocks . '[^>]*>)!', "\n$1", $pee );
	$pee = preg_replace( '!(</' . $allblocks . '>)!', "$1\n\n", $pee );

	/* sbf: take care of [response], [recaptcha], and [hidden] tags */
	...
	/* sbf: take care of [response], [recaptcha], and [hidden] tag */
	...
	if ( $br ) {
		...
		$pee = preg_replace_callback(
			...
			'sbf_autop_preserve_newline_callback', $pee );
		$pee = preg_replace( '|(?<!<br />)\s*\n|', "<br />\n", $pee ); // optionally make line breaks
		$pee = str_replace( '<WPPreserveNewline />', "\n", $pee );

		/* sbf: remove extra <br /> just added before [response], [recaptcha], and [hidden] tags */
...
function sbf_autop_preserve_newline_callback( $matches ) {
	return str_replace( "\n", '<WPPreserveNewline />', $matches[0] );
}

function sbf_sanitize_query_var( $text ) {
...
function sbf_strip_quote_deep( $arr ) {
	if ( is_string( $arr ) ) {
		return sbf_strip_quote( $arr );
	}
	...
	if ( is_array( $arr ) ) {
		...
		foreach ( $arr as $key => $text ) {
			$result[$key] = sbf_strip_quote_deep( $text );
		}
...
function sbf_normalize_newline( $text, $to = "\n" ) {
	if ( ! is_string( $text ) ) {
		return $text;
	}
...
function sbf_normalize_newline_deep( $arr, $to = "\n" ) {
	if ( is_array( $arr ) ) {
		$result = array();

		foreach ( $arr as $key => $text ) {
			$result[$key] = sbf_normalize_newline_deep( $text, $to );
		}

		return $result;
	}

	return sbf_normalize_newline( $arr, $to );
}

function sbf_strip_newline( $str ) {
	$str = (string) $str;
	$str = str_replace( array( "\r", "\n" ), '', $str );
	return trim( $str );
}

function sbf_canonicalize( $text, $strto = 'lower' ) {
 ...
 */
function sbf_is_name( $string ) {
	return preg_match( '/^[A-Za-z][-A-Za-z0-9_:.]*$/', $string );
}

function sbf_sanitize_unit_tag( $tag ) {
	$tag = preg_replace( '/[^A-Za-z0-9_-]/', '', $tag );
	return $tag;
}

function sbf_is_email( $email ) {
	$result = is_email( $email );
	return apply_filters( 'sbf_is_email', $result, $email );
}

function sbf_is_url( $url ) {
	$result = ( false !== filter_var( $url, FILTER_VALIDATE_URL ) );
	return apply_filters( 'sbf_is_url', $result, $url );
}

function sbf_is_tel( $tel ) {
	$pattern = '%^[+]?' // + sign
	...
	return apply_filters( 'sbf_is_tel', $result, $tel );
}

function sbf_is_number( $number ) {
	$result = is_numeric( $number );
	return apply_filters( 'sbf_is_number', $result, $number );
}

function sbf_is_date( $date ) {
	...
	return apply_filters( 'sbf_is_date', $result, $date );
}

function sbf_is_mailbox_list( $mailbox_list ) {
...
function sbf_is_email_in_domain( $email, $domain ) {
	$email_list = sbf_is_mailbox_list( $email );
...
function sbf_is_email_in_site_domain( $email ) {
	if ( sbf_is_localhost() ) {
		return true;
	}
	...
	if ( sbf_is_email_in_domain( $email, $site_domain ) ) {
		return true;
	}
	...
	if ( preg_match( '%^https?://([^/]+)%', $home_url, $matches ) ) {
		...
		and sbf_is_email_in_domain( $email, $site_domain ) ) {
			return true;
		}
	}
...
function sbf_antiscript_file_name( $filename ) {
...
function sbf_mask_password( $text, $length_unmasked = 0 ) {
	$length = strlen( $text );
