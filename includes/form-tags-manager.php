...
function sbf_add_form_tag( $tag, $func, $features = '' ) {
	$manager = SBF_FormTagsManager::get_instance();

	return $manager->add( $tag, $func, $features );
}

function sbf_remove_form_tag( $tag ) {
	$manager = SBF_FormTagsManager::get_instance();

	return $manager->remove( $tag );
}

function sbf_replace_all_form_tags( $content ) {
	$manager = SBF_FormTagsManager::get_instance();

	return $manager->replace_all( $content );
}

function sbf_scan_form_tags( $cond = null ) {
	$contact_form = SBF_ContactForm::get_current();
...
function sbf_form_tag_supports( $tag, $feature ) {
	$manager = SBF_FormTagsManager::get_instance();

	return $manager->tag_type_supports( $tag, $feature );
}

class SBF_FormTagsManager {
	...
	private function scan_callback( $m, $replace = false ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				...
				and ! empty( $attr['options'] ) ) {
					...
					if ( ! sbf_is_name( $scanned_tag['name'] ) ) {
			...
			if ( SBF_USE_PIPE ) {
				$pipes = new SBF_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'sbf_form_tag', $scanned_tag, $replace );

		$scanned_tag = new SBF_FormTag( $scanned_tag );
	...
	private function parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = sbf_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
