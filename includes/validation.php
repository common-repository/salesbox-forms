...
class SBF_Validation implements ArrayAccess {
	...
	public function invalidate( $context, $message ) {
		if ( $context instanceof SBF_FormTag ) {
			$tag = $context;
		} elseif ( is_array( $context ) ) {
			$tag = new SBF_FormTag( $context );
		} elseif ( is_string( $context ) ) {
			$tags = sbf_scan_form_tags( array( 'name' => trim( $context ) ) );
			$tag = $tags ? new SBF_FormTag( $tags[0] ) : null;
		}
