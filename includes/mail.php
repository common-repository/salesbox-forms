...
class SBF_Mail {
	...
	private function __construct( $name, $template ) {
		...
		if ( $submission = SBF_Submission::get_instance() ) {
	...
	private function htmlize( $body ) {
		...
		$header = apply_filters( 'sbf_mail_html_header',
			'<!doctype html>
...
', $this );
		...
		$footer = apply_filters( 'sbf_mail_html_footer',
			'</body>
</html>', $this );
	...
	private function compose( $send = true ) {
		...
		$components = apply_filters( 'sbf_mail_components',
			$components, sbf_get_current_contact_form(), $this
		);
		...
		$subject = sbf_strip_newline( $components['subject'] );
		$sender = sbf_strip_newline( $components['sender'] );
		$recipient = sbf_strip_newline( $components['recipient'] );
		...
		if ( $this->use_html ) {
			...
			$headers .= "X-SBF-Content-Type: text/html\n";
		} else {
			$headers .= "X-SBF-Content-Type: text/plain\n";
		}
	...
	public function replace_tags( $content, $args = '' ) {
		...
		return sbf_mail_replace_tags( $content, $args );
	}
...
function sbf_mail_replace_tags( $content, $args = '' ) {
	$args = wp_parse_args( $args, array(
		'html' => false,
		'exclude_blank' => false,
	) );
	...
	if ( is_array( $content ) ) {
		foreach ( $content as $key => $value ) {
			$content[$key] = sbf_mail_replace_tags( $value, $args );
		}
	...
	foreach ( $content as $num => $line ) {
		$line = new SBF_MailTaggedText( $line, $args );
...
add_action( 'phpmailer_init', 'sbf_phpmailer_init', 10, 1 );

function sbf_phpmailer_init( $phpmailer ) {
	$custom_headers = $phpmailer->getCustomHeaders();
	$phpmailer->clearCustomHeaders();
	$sbf_content_type = false;

	foreach ( (array) $custom_headers as $custom_header ) {
		$name = $custom_header[0];
		$value = $custom_header[1];

		if ( 'X-SBF-Content-Type' === $name ) {
			$sbf_content_type = trim( $value );
		} else {
			$phpmailer->addCustomHeader( $name, $value );
		}
	}

	if ( 'text/html' === $sbf_content_type ) {
		$phpmailer->msgHTML( $phpmailer->Body );
	} elseif ( 'text/plain' === $sbf_content_type ) {
		$phpmailer->AltBody = '';
	}
}

class SBF_MailTaggedText {
	...
	private function replace_tags_callback( $matches, $html = false ) {
		...
		if ( $mail_tag->get_option( 'do_not_heat' ) ) {
			$submitted = isset( $_POST[$field_name] ) ? sanitize_post($_POST[$field_name]) : '';
		}
		...
		if ( null !== $replaced ) {
			...
			$replaced = sbf_flat_join( $replaced );
		...
		if ( $form_tag = $mail_tag->corresponding_form_tag() ) {
			...
			$replaced = apply_filters(
				"sbf_mail_tag_replaced_{$type}", $replaced,
				$submitted, $html, $mail_tag
			);
		}

		$replaced = apply_filters(
			'sbf_mail_tag_replaced', $replaced,
			$submitted, $html, $mail_tag
		);
		...
		$special = apply_filters( 'sbf_special_mail_tags', null,
...
class SBF_MailTag {
	...
	public function __construct( $tag, $tagname, $values ) {
		...
		if ( ! empty( $values ) ) {
			...
			$this->values = sbf_strip_quote_deep( $matches[0] );
		}
	...
	public function corresponding_form_tag() {
		if ( $this->form_tag instanceof SBF_FormTag ) {
