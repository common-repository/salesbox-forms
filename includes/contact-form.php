...
class SBF_ContactForm {

	const post_type = 'sbf_contact_form';
	...
	public static function register_post_type() {
		register_post_type( self::post_type, array(
			'labels' => array(
				'name' => __( 'Contact Forms', 'salesbox-crm-form' ),
				'singular_name' => __( 'Contact Form', 'salesbox-crm-form' ),
			),
			'rewrite' => false,
			'query_var' => false,
			'public' => false,
			...
			'capabilities' => array(
				'edit_post' => 'sbf_edit_contact_form',
				'read_post' => 'sbf_read_contact_form',
				'delete_post' => 'sbf_delete_contact_form',
				'edit_posts' => 'sbf_edit_contact_forms',
				'edit_others_posts' => 'sbf_edit_contact_forms',
				'publish_posts' => 'sbf_edit_contact_forms',
				'read_private_posts' => 'sbf_edit_contact_forms',
			),
		) );
	}
	...
	public static function get_template( $args = '' ) {
		$args = wp_parse_args( $args, array(
			'locale' => '',
			'title' => __( 'Untitled', 'salesbox-crm-form' ),
		) );

		$locale = $args['locale'];
		$title = $args['title'];

		if ( ! $switched = sbf_load_textdomain( $locale ) ) {
		...
		foreach ( $properties as $key => $value ) {
			$properties[$key] = SBF_ContactFormTemplate::get_default( $key );
		}

		$contact_form->properties = $properties;

		$contact_form = apply_filters( 'sbf_contact_form_default_pack',
			$contact_form, $args
		);
	...
	private static function generate_unit_tag( $id = 0 ) {
		...
		if ( in_the_loop() ) {
			$unit_tag = sprintf( 'sbf-f%1$d-p%2$d-o%3$d',
				absint( $id ),
				get_the_ID(),
				$global_count
			);
		} else {
			$unit_tag = sprintf( 'sbf-f%1$d-o%2$d',
				absint( $id ),
				$global_count
			);
		}
	...
	private function __construct( $post = null ) {
		...
		do_action( 'sbf_contact_form', $this );
	}

	public function __get( $name ) {
		$message = __( '<code>%1$s</code> property of a <code>SBF_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'salesbox-crm-form' );
	...
	public function get_properties() {
		...
		$properties = (array) apply_filters( 'sbf_contact_form_properties',
			$properties, $this );
	...
	public function locale() {
		if ( sbf_is_valid_locale( $this->locale ) ) {
			return $this->locale;
		} else {
			return '';
		}
	...
	public function is_posted() {
		if ( ! SBF_Submission::get_instance() ) {
			return false;
		}

		if ( empty( $_POST['_sbf_unit_tag'] ) ) {
			return false;
		}

		return $this->unit_tag() === sanitize_text_field($_POST['_sbf_unit_tag']);
	}
	...
	public function form_html( $args = '' ) {
		...
		if ( 'raw_form' == $args['output'] ) {
			return '<pre class="sbf-raw-form"><code>'
				. esc_html( $this->prop( 'form' ) ) . '</code></pre>';
		}

		if ( $this->is_true( 'subscribers_only' )
		and ! current_user_can( 'sbf_submit', $this->id() ) ) {
			$notice = __(
				"This contact form is available only for logged in users.",
				'salesbox-crm-form' );
			$notice = sprintf(
				'<p class="sbf-subscribers-only">%s</p>',
				esc_html( $notice ) );

			return apply_filters( 'sbf_subscribers_only_notice', $notice, $this );
		}
		...
		$html = sprintf( '<div %s>',
			sbf_format_atts( array(
				...
				( get_option( 'html_type' ) == 'text/html' ) ? 'lang' : 'xml:lang'
					=> $lang_tag,
				'dir' => sbf_is_rtl( $this->locale ) ? 'rtl' : 'ltr',
			) )
		);

		$html .= "\n" . $this->screen_reader_response() . "\n";

		$url = sbf_get_request_uri();
		...
		$url = apply_filters( 'sbf_form_action_url', $url );

		$id_attr = apply_filters( 'sbf_form_id_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_id'] ) );

		$name_attr = apply_filters( 'sbf_form_name_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_name'] ) );

		$class = 'sbf-form wpcf7-form';
		...
		$class = implode( ' ', $class );
		$class = apply_filters( 'sbf_form_class_attr', $class );

		$enctype = apply_filters( 'sbf_form_enctype', '' );
		$autocomplete = apply_filters( 'sbf_form_autocomplete', '' );

		$novalidate = apply_filters( 'sbf_form_novalidate',
			sbf_support_html5()
		);
		...
		$atts = array(
			...
			'method' => 'post',
			'class' => $class,
			'enctype' => sbf_enctype_value( $enctype ),
	...
	private function form_hidden_fields() {
		$hidden_fields = array(
			'_sbf' => $this->id(),
			'_sbf_version' => SBF_VERSION,
			'_sbf_locale' => $this->locale(),
			'_sbf_unit_tag' => $this->unit_tag(),
			'_sbf_container_post' => 0,
			'_sbf_posted_data_hash' => '',
		);

		if ( in_the_loop() ) {
			$hidden_fields['_sbf_container_post'] = (int) get_the_ID();
		}

		if ( $this->nonce_is_active() && is_user_logged_in() ) {
			$hidden_fields['_wpnonce'] = sbf_create_nonce();
		}

		$hidden_fields += (array) apply_filters(
			'sbf_form_hidden_fields', array() );
	...
	public function form_response_output() {
		...
		$output = sprintf( '<div %1$s>%2$s</div>',
			sbf_format_atts( $atts ),
			esc_html( $content )
		);

		$output = apply_filters( 'sbf_form_response_output',
	...
	public function validation_error( $name ) {
		...
		return apply_filters( 'sbf_validation_error', $error, $name, $this );
	}

	/* Form Elements */

	public function replace_all_form_tags() {
		$manager = SBF_FormTagsManager::get_instance();
		$form = $this->prop( 'form' );
	...
	public function form_do_shortcode() {
		sbf_deprecated_function( __METHOD__, '4.6',
			'SBF_ContactForm::replace_all_form_tags' );
	...
	public function form_scan_shortcode( $cond = null ) {
		sbf_deprecated_function( __METHOD__, '4.6',
			'SBF_ContactForm::scan_form_tags'
		);
	...
	public function form_elements() {
		return apply_filters( 'sbf_form_elements',
	...
	public function collect_mail_tags( $args = '' ) {
		...
		return apply_filters( 'sbf_collect_mail_tags', $mailtags, $args, $this );
	}
	...
	public function submit( $args = '' ) {
		...
		$submission = SBF_Submission::get_instance( $this, array(
		...
		do_action( 'sbf_submit', $this, $result );
	...
	public function filter_message( $message, $status = '' ) {
		...
		$message = sbf_mail_replace_tags( $message, array( 'html' => true ) );
		$message = apply_filters( 'sbf_display_message', $message, $status );
	...
	public function nonce_is_active() {
		$is_active = SBF_VERIFY_NONCE;

		if ( $this->is_true( 'subscribers_only' ) ) {
			$is_active = true;
		}

		return (bool) apply_filters( 'sbf_verify_nonce', $is_active, $this );
	}
		...
		if ( $post_id ) {
			foreach ( $props as $prop => $value ) {
				update_post_meta( $post_id, '_' . $prop,
					sbf_normalize_newline_deep( $value ) );
			}
			...
			if ( $this->initial() ) {
				$this->id = $post_id;
				do_action( 'sbf_after_create', $this );
			} else {
				do_action( 'sbf_after_update', $this );
			}

			do_action( 'sbf_after_save', $this );
		}
	...
	public function copy() {
		...
		return apply_filters( 'sbf_copy', $new, $this );
	}
	...
	public function shortcode( $args = '' ) {
		...
		if ( $args['use_old_format'] ) {
			$old_unit_id = (int) get_post_meta( $this->id, '_old_sbf_unit_id', true );
			...
			$shortcode = sprintf( '[salesbox-crm-form id="%1$d" title="%2$s"]',
				$this->id, $title );
		}

		return apply_filters( 'sbf_contact_form_shortcode',
			$shortcode, $args, $this );
	}
}
