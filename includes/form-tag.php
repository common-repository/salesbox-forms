...
class SBF_FormTag implements ArrayAccess {
	...
	public function get_date_option( $opt ) {
		...
		$date = apply_filters( 'sbf_form_tag_date_option',
			null,
	...
	public function get_default_option( $default = '', $args = '' ) {
		...
		foreach ( $options as $opt ) {
			...
			and is_user_logged_in() ) {
				...
				$vals = array_map( 'sbf_sanitize_query_var', $vals );
				...
				if ( $contact_form = SBF_ContactForm::get_current() ) {
	...
	public function get_data_option( $args = '' ) {
		...
		return apply_filters( 'sbf_form_tag_data_option', null, $options, $args );
	}
