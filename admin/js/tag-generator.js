( function( $ ) {
	...
	$( 'form.tag-generator-panel .control-box :input' ).change( function() {
		...
		sbf.taggen.normalize( $( this ) );
		sbf.taggen.update( $form );
	} );

	$( 'input.insert-tag' ).click( function() {
		var $form = $( this ).closest( 'form.tag-generator-panel' );
		var tag = $form.find( 'input.tag' ).val();
		sbf.taggen.insert( tag );
		tb_remove(); // close thickbox
		return false;
	} );

	sbf.taggen.update = function( $form ) {
		...
		if ( $.isFunction( sbf.taggen.update[ id ] ) ) {
			return sbf.taggen.update[ id ].call( this, $form );
		}
		...
		$form.find( 'input.tag' ).each( function() {
			...
			var components = sbf.taggen.compose( tag_type, $form );
			$( this ).val( components );
		} );
	...
	sbf.taggen.update.captcha = function( $form ) {
		var captchac = sbf.taggen.compose( 'captchac', $form );
		var captchar = sbf.taggen.compose( 'captchar', $form );

		$form.find( 'input.tag' ).val( captchac + ' ' + captchar );
	};

	sbf.taggen.compose = function( tagType, $form ) {
	...
	sbf.taggen.normalize = function( $input ) {
		var val = $input.val();
		...
		if ( $input.is( ':checkbox.exclusive' ) ) {
			sbf.taggen.exclusiveCheckbox( $input );
		}
	};

	sbf.taggen.exclusiveCheckbox = function( $cb ) {
		if ( $cb.is( ':checked' ) ) {
			$cb.siblings( ':checkbox.exclusive' ).prop( 'checked', false );
		}
	};

	sbf.taggen.insert = function( content ) {
		$( 'textarea#sbf-form' ).each( function() {
			this.focus();
